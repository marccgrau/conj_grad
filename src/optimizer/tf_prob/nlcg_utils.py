...
ValueAndGradient = collections.namedtuple('ValueAndGradient', ['x', 'f', 'df']) #, 'full_gradient'])
...
def terminate_if_not_finite(state, value=None, gradient=None):
    ...
    - If the gradient has any nonfinite values, we can't use it to move a finite
        amount.
    Args:
        state: A NlcgOptimizerResults representing the
...
def line_search_step(state, value_and_gradients_function, search_direction,
    ...
    """Performs the line search step of the NLCG search procedure.
    ...
    Args:
        ...
        search procedure. The state must include the fields: `position`,
            `objective_value`, `objective_gradient`, `num_iter`,
        ...
        max_iter: A Python integer that is used as the maximum number of
    ...
    state_after_ls = update_fields(
        ...
        num_iter=state.num_iter + 1,
        num_objective_evaluations=(state.num_objective_evaluations + ls_result.func_evals))
    ...
    def _do_update_position():
        ...
        return _update_position(
            state_after_ls, # add up number of objective evals, func evals, iterations, failed state
            position_delta, # optimal step along search direction
            ls_result.left.f, # objective value at left end of interval
            ls_result.left.full_gradient, # gradient at left end of interval
            grad_tolerance, f_relative_tolerance, x_tolerance,
...
def _update_position(state,
                     position_delta,
                     next_objective, # left.f from hager zhang
                     next_gradient, # left.full_gradient from hager_zhang
                     grad_tolerance,
    ...
    # position_delta is the optimal step size alpha times the search direction.
    # x_{i+1} = x_i + alpha_i * d_i
    next_position = state.position + position_delta
    # pyformat: disable
    # check if there is an actual convergence given the initial position x_i and candidate position x_{i+1}
    ...
    # pyformat: enable
    # Update the state with the new position, objective value and gradient.
...
def _check_convergence(current_position,
    ...
    f_relative_converged = (
        norm(next_objective - current_objective, dims=0) <= f_relative_tolerance * current_objective)
